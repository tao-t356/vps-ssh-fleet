...
- `3` TaoBox Speed 节点部署
- `4` VLESS + Hysteria2 节点搭建
- `5` Docker + Nginx Proxy Manager 安装
- `6` Docker 容器管理
- `7` 网络工具 / BBR
- `8` 系统工具 / DD
- `9` 更新工具箱
...
- 选择 `3` 会**直接运行** `scripts/taobox-speed.sh --install-argo-vmess`
- 选择 `4` 会**直接运行** `vless-xhttp-reality-self`
- 选择 `5` 会**直接运行** `Docker + Nginx Proxy Manager`
...
- `TaoBox Speed`：偏 **系统调优 + Argo VMess WS 节点一体化**
- `VLESS + Hysteria2`：偏 **你原来的代理协议栈部署**
...
选好系统后，输入一次新 `root` 密码就会开始执行。

> 当前版本工具箱内部已经内置默认 `jshook: 123`，进入菜单后一般不需要再重复输入。
